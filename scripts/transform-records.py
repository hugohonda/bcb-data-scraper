...
data = json.load(open('../output/records-raw-1998-2018.json'))
...
for record in data:
  ...
  try:
    if record['id'] < 200:
      record['topics'] = get_topics(text)
    else:
      record['topics'] = get_topics_200(text)
  except Exception:
...
if not os.path.isdir(output_dir):
  os.mkdir(output_dir)
with open(f'{output_dir}/records-processed-{initial}-{final}.json', 'w') as file:
  json.dump(data, file)
